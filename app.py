...
with st.sidebar:
    ...
    page = st.radio(
        "Select Page:",
        ["🏠 Route Grievance", "📊 Analytics", "📚 Knowledge Graph", "📋 History", "ℹ️ About"]
    )
...
if page == "🏠 Route Grievance":
    ...
    with col1:
        ...
        st.markdown("Describe your issue. Our Knowledge Graph will analyze it and route to the right authority.")
    ...
    grievance_text = st.text_area(
        ...
        height=150,
        placeholder="""Example: My PM-Kisan payment has not come because my Aadhaar 
name doesn't match my bank account. The bank says to contact Aadhaar, 
and Aadhaar says to contact the bank. I'm not receiving any money.""",
        key="grievance_input"
    )
    ...
    if preset != "Custom" and not grievance_text:
        templates = {
            ...
            "Aadhaar Mismatch": "My Aadhaar name is 'Rajesh Kumar Singh' but my bank account has 'R K Singh'. The payment system is rejecting my PM-Kisan payments due to this mismatch.",
            "Bank Account Linking Failed": "I linked my Aadhaar to my bank account but the DBT system still says my account is not verified. I cannot receive my PM-Kisan payment.",
    ...
    with col1:
        applicant_name = st.text_input("Your Name (Optional)", key="name_input")
    
    with col2:
        state = st.selectbox(
            "State",
            ["Select", "Uttar Pradesh", "Maharashtra", "Bihar", "Punjab", "Madhya Pradesh", "Other"],
            key="state_select"
        )
    ...
    # Submit button
    if st.button("🚀 Analyze & Route", key="submit_btn", use_container_width=True):
        
        if not grievance_text.strip():
            st.error("❌ Please enter your grievance")
        else:
            with st.spinner("🔍 Analyzing grievance using Knowledge Graph..."):
                try:
                    # Route the grievance
                    routing_decision = st.session_state.router.route_grievance(grievance_text)
                    ...
                    with tab1:
                        ...
                        if routing_decision.failure_types:
                            for i, failure in enumerate(routing_decision.failure_types, 1):
                    ...
                    with tab3:
                        ...
                        if routing_decision.graph_path:
                            ...
                            st.info("Graph path construction in progress...")
                    ...
                    with tab5:
                        ...
                        st.json({
                            ...
                            'resolutions_count': len(routing_decision.resolution_paths)
                        })
                    ...
                    with col1:
                        if st.button("📧 Email to Department", use_container_width=True):
                            st.success("✅ Email sent to department!")
                    
                    with col2:
                        ticket_id = f"GR-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                        if st.button("🔗 Generate Ticket", use_container_width=True):
    ...
    if all_schemes:
        ...
        if selected_scheme:
            ...
            if scheme:
                ...
                with col2:
                    all_failures = st.session_state.db.get_all_failure_types()
                    st.metric("Possible Failures", len(all_failures) if all_failures else 0)
