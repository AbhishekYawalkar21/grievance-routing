...
class SupabaseGraphClient:
    ...
    def get_services_by_scheme(self, scheme_id: int) -> List[Dict]:
        ...
        response = self.client.table('scheme_service').select(
            'service_id, services(id, name, service_type)'
        ).eq('scheme_id', scheme_id).execute()
        
        return [item['services'] for item in response.data if item['services']]
    
    def get_failures_by_service(self, service_id: int) -> List[Dict]:
        """Get possible failure types for a service"""
        
        response = self.client.table('service_failure').select(
            'failure_id, failure_types(id, name, severity, category)'
        ).eq('service_id', service_id).execute()
        
        return [item['failure_types'] for item in response.data if item['failure_types']]
    
    def get_resolutions_by_failure(self, failure_id: int) -> List[Dict]:
        """Get resolution paths for a failure"""
        
        response = self.client.table('failure_resolution').select(
            'resolution_id, resolution_paths(id, name, steps, time_to_resolve, authority)'
        ).eq('failure_id', failure_id).execute()
        
        return [item['resolution_paths'] for item in response.data if item['resolution_paths']]
    
    def get_authorities_by_resolution(self, resolution_id: int) -> List[Dict]:
        """Get authorities responsible for resolution"""
        
        response = self.client.table('resolution_authority').select(
            'authority_id, appeal_authorities(id, name, level, contact)'
        ).eq('resolution_id', resolution_id).execute()
        
        return [item['appeal_authorities'] for item in response.data if item['appeal_authorities']]
    
    def get_authorities_by_department(self, department_id: int) -> List[Dict]:
        """Get appeal authorities for a department"""
        
        response = self.client.table('department_authority').select(
            'authority_id, appeal_authorities(id, name, level, contact)'
    ...
    def get_department_by_scheme(self, scheme_id: int) -> Optional[Dict]:
        ...
        response = self.client.table('schemes').select(
            'department_id, departments(id, name, state)'
        ).eq('id', scheme_id).execute()
        
        if response.data and response.data[0]['departments']:
    ...
    def log_grievance(self, grievance_text: str, owner_dept_id: int, 
                     confidence: float, failure_types: List[str], 
                     explanation: str) -> Dict:
        """Log routed grievance for audit trail"""
        
        response = self.client.table('grievance_audit').insert({
    ...
    def get_audit_summary(self) -> Dict:
        ...
        for audit in audits:
            dept_id = audit['owner_department_id']
    ...
    def get_audit_history(self, limit: int = 50) -> List[Dict]:
        ...
        response = self.client.table('grievance_audit').select('*').order(
